...
class ESDB(nn.Module):
    def __init__(self, in_channels, out_channels, conv=nn.Conv2d, p=0.25):
        ...
        self.c1_d = nn.Conv2d(in_channels*2, self.rc, 1)
        self.c1_r = conv(in_channels, self.rc, kernel_size=3,  **kwargs)
        
        self.c2_d = nn.Conv2d(self.remaining_channels*2, self.rc, 1)
        self.c2_r = conv(self.remaining_channels, self.rc, kernel_size=3, **kwargs)
        
        self.c3_d = nn.Conv2d(self.remaining_channels*2, self.rc, 1)
...
class HADNS(nn.Module):
    def __init__(self, num_in_ch=3, num_feat=48, num_block=5, num_out_ch=3, upscale=4,
                 conv='BSConvU', upsampler='pixelshuffledirect', p=0.25):
        super(HADNS, self).__init__()
        kwargs = {'padding': 1}
        if conv == 'BSConvS':
            kwargs = {'p': p}
